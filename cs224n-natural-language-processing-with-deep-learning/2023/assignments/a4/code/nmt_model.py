...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = pack_padded_sequence(X.permute((2,0,1)), source_lengths)  # (src_len, b, e)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)       # (src_len, b, h*2), (2, b, h), (2, b, h)
        enc_hiddens, _ = pad_packed_sequence(sequence=enc_hiddens)    # (src_len, b, h*2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.einsum("blh, bh->bl", enc_hiddens_proj, dec_hidden)          # (b, src_len)
