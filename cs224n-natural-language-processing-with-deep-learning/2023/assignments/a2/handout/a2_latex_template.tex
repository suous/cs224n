...
\begin{answer}
    ...
    Classify phrases as being positive or negative is a binary classification problem.
    The output layer of the model should be a sigmoid function, and the loss function should be the binary cross-entropy loss function.

    \begin{equation}
        \begin{align}
            \hat{\bm y} &= \sigma(\bm w \bm u) \\
            J(\bm u,\bm w) &= -\sum_{k=1}^{n} y_k\log{\hat{y}_k} + (1-y_k)\log{(1-\hat{y}_k)}
        \end{align}\label{eq:1b7}
    \end{equation}

    \begin{enumerate}
        \item For the well trained word2vec model with large word vector dimension, the L2 normalization will not affect the performance of the model too much. Because it is unlikely that the word vector will change it's direction too much after the normalization. \\
        \item For the model with smaller word vector dimension, the L2 normalization will affect the performance of the model. Because the word meaning may change after the normalization. For example $\mathbf{u}_x =\alpha \mathbf{u}_y$ after the normalization, where $\alpha$ is a constant.
    \end{enumerate}
...
\begin{answer}
    ...
    \begin{equation}
        \begin{align}
            \frac{\partial{J(\bm v_c,o,\bm U)}}{\partial{\bm u_w}} &= \frac{1}{\sum_{w\in{Vocab}}exp(\bm u_w^T\bm v_c)}\cdot\frac{\partial{\sum_{x\in{Vocab}}exp(\bm v_c\bm u_x^T)}}{\partial{\bm u_w}} - \bm v_c  \\
            &=\frac{\sum_{x\in{Vocab}}exp(\bm v_c\bm u_x^T) \cdot \bm v_c}{\sum_{w\in{Vocab}}exp(\bm v_c\bm u_w^T)} - \bm v_c \\
            &=\sum_{x\in{Vocab}} \frac{exp(\bm v_c\bm u_x^T)}{\sum_{w\in{Vocab}}exp(\bm v_c\bm u_w^T)} \cdot \bm v_c - \bm v_c \\
            &=\sum_{x\in{Vocab}} P(O=x|C=c) \cdot \bm v_c - \bm v_c \\
            &= \hat{\bm y}^T \cdot \bm v_c - \bm v_c
        \end{align}\label{eq:1c1}
    \end{equation}
    ...
    \begin{equation}
        \begin{align}
            \frac{\partial{J(\bm v_c,o,\bm U)}}{\partial{\bm u_w}} &= \frac{1}{\sum_{w\in{Vocab}}exp(\bm v_c\bm u_w^T)}\cdot\frac{\partial{\sum_{x\in{Vocab}}exp(\bm v_c\bm u_x^T)}}{\partial{\bm u_w}}  \\
            &=\frac{\sum_{x\in{Vocab}}exp(\bm v_c\bm u_x^T) \cdot \bm v_c}{\sum_{w\in{Vocab}}exp(\bm v_c\bm u_w^T)}  \\
            &=\sum_{x\in{Vocab}} \frac{exp(\bm v_c\bm u_x^T)}{\sum_{w\in{Vocab}}exp(\bm v_c\bm u_w^T)} \cdot \bm v_c  \\
            &=\sum_{x\in{Vocab}} P(O=x|C=c) \cdot \bm v_c  \\
            &= \hat{\bm y}^T \cdot \bm v_c
        \end{align}\label{eq:1c2}
    \end{equation}
...
\begin{answer}
    ...
    \begin{equation}
        \begin{align}
            \frac{\partial{J(\bm v_c,o,\bm U)}}{\partial{\bm u_{w_s}}} &= - \sum^K_{\substack{j=1\\w_j= w_s}}\frac{1}{ \sigma(-\bm v_c\bm u_{w_s}^T)}\cdot\frac{\partial{ \sigma(-\bm v_c\bm u_{w_s}^T)}}{\partial{\bm u_{w_s}}} \\
            &= - \sum^K_{\substack{j=1\\w_j= w_s}}\frac{1}{ \sigma(-\bm v_c\bm u_{w_s}^T)}\cdot \sigma(-\bm v_c\bm u_{w_s}^T) \left(1-\sigma(-\bm v_c\bm u_{w_s}^T)\right) \cdot (-\bm v_c) \\
            &= \sum^K_{\substack{j=1\\w_j= w_s}}\left(1-\sigma(-\bm v_c\bm u_{w_s}^T)\right) \cdot \bm v_c
        \end{align}\label{eq:1h2}
    \end{equation}
